function FlevaR(_div = document.body, _options = {}) {
        ...
        Engine: function Engine() {
            for (const args of arguments)
                for (const arg of Object.keys(args))
                    defineConstructor(this, arg, args[arg]);
        },
    ...
    const __callRenderFunction = () => {
        if (helperUtils.isScriptOrFunction(__renderFunction)) {
            __commandUtils.getScriptOrFunction(__renderFunction)(____thisObj, ____engine);
        }
    }
    ...
    const __library = {
        instances: {}, scripts: {}, scenes: {}, sprites: {}, graphics: {}, spritesheets: {}, paintings: {}, sounds: {},

        hasInstance: function (_name) {
    ...
    const propertyUtils = {
        listInstances: () => {
            const temp = [];
            for (const i of Object.keys(__library.instances))
                temp.push(`${i}->${__library.instances[i].type}`);
            return temp;
        },
        listScripts: () => {
    ...
    const __commandUtils = {
        ...
        renderInstance: ({ _bounds, _clip, ..._props }, _renderFunc, _parameters = []) => {
            const { ctx } = __screen;
        ...
        drawPainting: ({ _x = 0, _y = 0, _width = 50, _height = 50 } = {}, _painting) => {
            if (helperUtils.isString(_painting)) {
                ...
                src(__screen.ctx, _x, _y, _width, _height, ____engine);
    ...
    const ___errorsM = {
        ...
        checkInstanceExist: function (_name, _obj) {
            this.checkCanUseName(_name, "instance");
            if (_obj) {
                if (_obj[_name]) throw `Instance already exists: "${_name}".`;
            } else
                if (__library.hasInstance(_name)) throw `Instance already exists: "${_name}".`;
        },
        checkInstanceNotExist: function (_name, _obj) {
            if (_obj) {
                if (!_obj[_name]) throw `Instance does not exist: "${_name}".`;
            } else
                if (!__library.hasInstance(_name)) throw `Instance does not exist: "${_name}".`;
        },
    ...
    const __Scene = function (_inits) {
        ...
        const __callRenderFunction = () => {
            if (helperUtils.isScriptOrFunction(__renderFunction)) {
                __commandUtils.getScriptOrFunction(__renderFunction)(____renderObj, ____engine);
            }
        }
        const __heirarchy = {
            scripts: [], instances: {},
        ...
        __stack.removeFromMainStack = () => {
            for (const _name of Object.keys(__stack.list)) {
                const instance = __stack.list[_name];
                const index = __mainStack.list.findIndex(elem => elem.stackName === instance.stackName);
        ...
        const __addInstance = (_instanceType, _instanceCount, _instanceName, _preserve, _props, _inits) => {
            ...
            const Instance = _instanceType === "prefab" ? __Prefab : _instanceType === "textfield" ? __TextField : false;

            __heirarchy.instances[_instanceCount] = {
                stackName: _stackName,
                instanceName: _instanceName,
                preserve: _preserve,
                instance: Instance(_props, _inits, _funcs)
            }
            __stack.list[__stack.list.length] = {
                stackName: _stackName,
                depth: 1,
                instance: __heirarchy.instances[_instanceCount].instance,
                render: function () {
        ...
        const addInstance = (_instanceType, ..._args) => {
            ...
            if (helperUtils.isString(_args[0])) {
                ...
                if (!(__heirarchy.instances[__instanceCount] && __heirarchy.instances[__instanceCount].preserve && preserve)) {
                    const originalPrefabSkeleton = __getInstanceFromLibrary(_name).__private__;
                    const instanceType = originalPrefabSkeleton.___getType();

                    if (_instanceType !== instanceType) throw `Expected ${_instanceType} instance but received ${instanceType} instead.`;
                    ...
                    __addInstance(_instanceType, __instanceCount, instanceName, preserve, { ..._oldProps, ..._props }, _init);
                }
                __instanceCount++;
                ...
                if (!(__heirarchy.instances[__instanceCount] && __heirarchy.instances[__instanceCount].preserve && preserve)) {
                    __addInstance(_instanceType, __instanceCount, instanceName, preserve, _props, _init);
                }
                __instanceCount++;
        ...
        const addTextField = (...args) => {
            addInstance("textfield", ...args);
        }
        const addPrefab = (...args) => {
            addInstance("prefab", ...args);
        }
        ...
        const renderer = (_script) => {
            if (helperUtils.isString(_script)) {
                const _newScript = getScriptFromLibrary(_script);
                __renderFunction = __Script(_newScript).bind(____renderObj);
            } else if (helperUtils.isScriptOrFunction(_script)) {
                const _newScript = __commandUtils.getScriptOrFunction(_script);
                __renderFunction = __Script(_newScript).bind(____renderObj);
            }
        ...
        const load = async () => {
            ...
            if (inits.length > 0)
                for (const init of Object.keys(inits)) {
                    ...
                    const fini = await _init(____thisObj, ____engine);
                    if (helperUtils.isScriptOrFunction(fini)) finis.push(__commandUtils.getScriptOrFunction(fini));
                }

            __stack.addToMainStack();

            for (const _name of Object.keys(__heirarchy.instances)) {
                await __heirarchy.instances[_name].instance.__private__.__load();
            }
        ...
        const unload = async () => {
            ...
            if (finis.length > 0) {
                for (const fini of Object.keys(finis)) {
                    ...
                    await _fini(____thisObj, ____engine);
                }
                finis.length = 0;
            }
            ...
            for (const _name of Object.keys(__heirarchy.instances)) {
                if (!__heirarchy.instances[_name].preserve) {
                    const instance = __heirarchy.instances[_name];
                    const index = __stack.list.findIndex(elem => elem.stackName === instance.stackName);

                    if (index !== -1) {
                        __stack.list.splice(index, 1);
                    }

                    await instance.instance.__private__.__unload();
                    delete __heirarchy.instances[_name];
                }
            }
            __instanceCount = 0;
        ...
        const tick = async function () {
            for (const _name of Object.keys(__heirarchy.scripts)) {
                ...
                await _script(____thisObj, ____engine);
            }

            for (const _name of Object.keys(__heirarchy.instances)) {
                if (helperUtils.isDefined(__heirarchy.instances[_name])) await __heirarchy.instances[_name].instance.__private__.__tick();
            }
        ...
        const start = function () {
            for (const _name of Object.keys(__heirarchy.instances)) {
                __heirarchy.instances[_name].instance.__private__.__start();
            }
        }
        const stop = function () {
            for (const _name of Object.keys(__heirarchy.instances)) {
                __heirarchy.instances[_name].instance.__private__.__stop();
            }
        }
        ...
        const __private__ = {
            ___getInstanceByName: (_instanceName) => {
                for (const _name of Object.keys(__heirarchy.instances)) {
                    if (__heirarchy.instances[_name].instanceName === _instanceName) {
                        return __heirarchy.instances[_name].instance;
                    }
                }
                return undefined;
            },
                ...
                Object.defineProperties(__returns,
                    {
                        ...
                        _focused: {
                            ...
                            set(val) {
                                ...
                                if (!alreadySame)
                                    if (val) {
                                        __clearSelectedInstance();
                                        __setSelectedInstance(navigation.unselect);
                                    } else
                                        __clearSelectedInstance();
                            },
                            enumerable: true
        ...
        if (__private.__type !== "dynamic") {
            function setupNavigation() {
                ...
                function setCustomClipboard(str) {
                    NativeModule.clipboard = str;
                }
                function getCustomClipboard() {
                    return NativeModule.clipboard;
                }
            }
            setupNavigation();
        }
    ...
    const __TextField = function (_props, _inits, _funcs) {
        ...
        const hitTestInstance = (_instanceOrName, _pixel) => {
            let instance;
            if (helperUtils.isInstance(_instanceOrName))
                instance = _instanceOrName;
            else if (helperUtils.isString(_instanceOrName))
                instance = __getInstanceByName(_instanceOrName);
            else throw `Invalid instance argument! Instance or string expected.`;
            if (!instance) throw `Instance with instance name ${_instanceOrName} not found.`;

            const sourceProps = props;
            const targetProps = instance.__private__.___getProps();
            const sourceBounds = __getBounds();
            const targetBounds = instance.__private__.___getBounds();
        ...
        const render = () => {
            __commandUtils.renderInstance({ ...props, _bounds: __getBounds() }, __privateProps.__render);
        }
        const start = () => {
        }
        const stop = () => {
        ...
        const __private__ = {
            ...
            _isMouseInInstance: isMouseInInstance,
            __start: start, __stop: stop,
            __load: load, __unload: unload,
            __tick: tick, __render: render,
        }
    ...
    const __Prefab = function (_props = { _x: 0, _y: 0, _width: 50, _height: 50, _alpha: 1, _visible: true, _rotation: 0, _anchorX: 0, _anchorY: 0 }, _inits, _funcs) {
        ...
        const render = () => {
            ...
            if (__heirarchy.visuals.type === helperUtils.typeOf(__constructors.Sprite) ||
                __heirarchy.visuals.type === helperUtils.typeOf(__constructors.SpriteSheet)) {
                __commandUtils.renderInstance({ ...props, _bounds: __getBounds(), _clip }, __commandUtils.drawSprite, [__heirarchy.visuals.src]);
            } else if (__heirarchy.visuals.type === helperUtils.typeOf(__constructors.Graphic)) {
                __commandUtils.renderInstance({ ...props, _bounds: __getBounds(), _clip }, __commandUtils.drawGraphic, [__heirarchy.visuals.src]);
            } else if (__heirarchy.visuals.type === helperUtils.typeOf(__constructors.Painting)) {
                __commandUtils.renderInstance({ ...props, _bounds: __getBounds(), _clip }, __commandUtils.drawPainting, [__heirarchy.visuals.src]);
            }
        }
        const start = () => {
            __callRenderFunction();
                ...
                if (helperUtils.isSpriteSheet(_definition)) {
                    ...
                    if (helperUtils.isScriptOrFunction(_definition)) {
                        try {
                            ...
                            _definition(sctx, sX, sY, sW, sH, ____engine);
                        ...
                        try {
                            ...
                            src = src.bind(sctx);
                            src(sctx, sX, sY, sW, sH, ____engine);
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (helperUtils.isScriptOrFunction(_definition)) {
                try {
                    ...
                    _definition(ctx, sX, sY, sW, sH, ____engine);
                ...
                try {
                    ...
                    src = src.bind(ctx);
                    src(ctx, sX, sY, sW, sH, ____engine);
    ...
    const __getInstanceFromLibrary = function (_name) {
        ___errorsM.checkInstanceNotExist(_name);
        return __library.instances[_name].instance;
    }
    const __getInstanceByName = (_instanceName) => {

        if (heirarchy.scene !== "") {
            const instance = __library.scenes[heirarchy.scene].__private__.___getInstanceByName(_instanceName);
            if (instance) return instance;
        }


        for (const _name of Object.keys(__attachedInstances)) {
            if (__attachedInstances[_name].instanceName === _instanceName) {
                return __attachedInstances[_name].instance;
            }
        }
        return {};
    }
    ...
    const createInstance = function (_instanceType, ..._args) {
        const _name = _args[0];
        ___errorsM.checkInstanceExist(_name);
        ...
        const Instance = _instanceType === "prefab" ? __Prefab : _instanceType === "textfield" ? __TextField : false;
        __library.instances[_name] = {
            instance: Instance(props, init),
            type: _instanceType
        };
    }

    const __attachedInstances = {};
    const __attachInstance = async (_instanceType, _instanceName, _instName, _props, _inits) => {
        const _stackName = __generateSymbolName("stack");
        const _swapDepths = (_depth) => __mainStack.swapDepths(_stackName, _depth);
        const _funcs = { swapDepths: _swapDepths };
        ...
        __attachedInstances[_instanceName] = {
            stackName: _stackName,
        ...
        __mainStack.list[__mainStack.list.length] = {
            stackName: _stackName,
            depth: 1,
            instance: __attachedInstances[_instanceName].instance,
            render: function () {
                this.instance.__private__.__render();
            }
        }
        ...
        __mainStack.sortList();
        await __attachedInstances[_instanceName].instance.__private__.__load();
    }
    const attachInstance = async (_instanceType, ..._args) => {
        ...
        if (helperUtils.isString(_args[0])) {
            ...
            await __attachInstance(_instanceType, _instanceName, instanceName, { ..._oldProps, ..._props }, _init);
        } else if (helperUtils.isObject(_args[0]) && helperUtils.isScriptOrFunction(_args[1])) {
            modifiedProps = _args[0] || {};
            const _init = __commandUtils.getScriptOrFunction(_args[1]);
            const { instanceName: __instanceName, preserve: __preserve = false, ...__props } = modifiedProps;
            ...
            await __attachInstance(_instanceType, _instanceName, instanceName, _props, _init);
        } else if (helperUtils.isScriptOrFunction(_args[0])) {
            const _init = __commandUtils.getScriptOrFunction(_args[0]);
            const { instanceName: __instanceName, preserve: __preserve = false, ...__props } = modifiedProps;
            [instanceName, preserve, _props] = [__instanceName, __preserve, __props];
    ...
    const removeInstance = async function (_instanceType, _instanceName) {
        if (__attachedInstances[_instanceName]) {
            const instance = __attachedInstances[_instanceName];
            const instanceType = instance.instance.__private__.___getType();
            if (_instanceType !== instanceType) return;
            ...
            await instance.instance.__private__.__unload();
            delete __attachedInstances[_instanceName];
        }
    ...
    let ___selectedInstance = undefined;
    const __hasSelectedInstance = () => {
        return ___selectedInstance !== undefined;
    }
    const __setSelectedInstance = (_state) => {
        ___selectedInstance = _state;
    }
    const __clearSelectedInstance = () => {
        if (__hasSelectedInstance()) __setSelectedInstance(___selectedInstance());
    }
    const __checkSelectedInstance = () => {
        if (MouseModule.isPressed("left")) {
            ...
            for (const _key of Object.keys(stack)) {
                ...
                if (instance.__private__.___setSelection)
                    if (instance.__private__._isMouseInInstance(stage._xmouse, stage._ymouse)) {
                        __setSelectedInstance(instance.__private__.___setSelection(stage._xmouse, stage._ymouse));
                        if (__hasSelectedInstance()) break;
                    }
            }

            if (__hasSelectedInstance()) {
                __clearMouseStates();
                __clearKeyStates();
            }
        }
    ...
    const __tick = async function () {
        ...
        for (const _name of Object.keys(__attachedInstances)) {
            await __attachedInstances[_name].instance.__private__.__tick();
        }
    ...
    const __render = function () {
        ...
        for (const _name of Object.keys(__mainStack.list)) {
            const instance = __mainStack.list[_name];
            instance.render();
        }
    }
    ...
    const startRunTime = function (_interval) {
        ...
        for (const _name of Object.keys(__attachedInstances)) {
            __attachedInstances[_name].instance.__private__.__start();
        }
    ...
    const stopRunTime = function () {
        ...
        for (const _name of Object.keys(__attachedInstances)) {
            __attachedInstances[_name].instance.__private__.__stop();
        }
    ...
    const NativeModule = (function NativeModule() {
        ...
        Object.defineProperties(____returns, {
            ...
            FLEVAR_ENV: {
                get: function () { return __defaults.flevar_env; },
                enumerable: false,
                configurable: false
            }
        });
    ...
    const SharedObjectModule = (function SharedObjectModule() {
        ...
        const IDB = "indexeddb";
        const drivers = {};
        const driver = (function defineDriver() {
            if (!window.indexedDB || __defaults.autosave) return LS;
            return IDB;
        })();
    ...
    const stage = (() => {
        ...
        Object.defineProperties(____returns, {
            ...
            _scene: {
                get: function () { return getCurrentScene(); },
                enumerable: false,
                configurable: false
            },
            _xmouse: {
    ...
    const _root = new (function Root() {
        return new Proxy(this, {
            get: function (object, property) {
                return __getInstanceByName(property);
            }
        });
    })();
    ...
    const attachPrefab = async (..._args) => {
        await attachInstance("prefab", ..._args);
    }
    const attachTextField = async (..._args) => {
        await attachInstance("textfield", ..._args);
    }

    const removePrefab = function (_instanceName) {
        removeInstance("prefab", _instanceName);
    }
    const removeTextField = function (_instanceName) {
        removeInstance("textfield", _instanceName);
    }

    const createPrefab = function (_name, _props, _init) {
        createInstance("prefab", _name, _props, _init)
    }
    const createTextField = function (_name, _props, _init) {
        createInstance("textfield", _name, _props, _init)
    }
        ...
        const __simulateLoad = () => {
            return new Promise(async (resolve, reject) => {
                try {
                    ...
                    onLoad = await __loadToQueue(__loaderObj, ____engine);
